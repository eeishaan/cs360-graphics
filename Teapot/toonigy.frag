...
{
 return texture(texUnit,coords + vec2(dx, dy));
}

// returns pixel color
float IsEdge(in vec2 coords)
{
  float dxtex = 1.0 /float(textureSize(texUnit,0)) ;
  float dytex = 1.0 /float(textureSize(texUnit,0));
  float pix[9];
  int k = -1;
  float delta;
...
void main()
{
 ...
 /* if (uv.x > (mouse_x_offset+0.002))
  {*/
    vec3 colorOrg = texture(texUnit, uv).rgb;
